...
public:
	...
	template<typename... Args> requires (sizeof...(Args) <= height * width) && (... && (is_floating_type_v<Args> || is_integral_type_v<Args>))
	...
	void SetCell(const size_t& row, const size_t& col, const type& data);
	...
	Matrix<type, width, height> Inverse() const requires(height != width);
...
template <typename type, size_t height, size_t width>
template <size_t otherWidth> requires (otherWidth >= 1)
Matrix<type, height, otherWidth> Matrix<type, height, width>::MatrixProduct(const Matrix<type, width, otherWidth>& other)
{
